...
public final class ServletContainer {
    private static Map<String, Class<HttpServlet>> servletMap;

    // lazy init when have httpServlet.
    // new httpServlet will replace old httpServlet if have same path.
    public static void put(String path, Class<HttpServlet> httpServlet) {
        if (servletMap == null) {
    ...
    public static Class<HttpServlet> get(String path) {
        if (servletMap == null) {
            return null;
        }
